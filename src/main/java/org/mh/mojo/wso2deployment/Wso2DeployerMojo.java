...
public class Wso2DeployerMojo extends AbstractDeployerMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        for (final Serverconfig svr : environment) {
            getLog().info("[WSO2 Deployer Mojo] Server (" + svr.getServerId() + ") URL: " + svr.getServerUrl());
        }
        for (final Deployment deploy : deployments) {
            getLog().info("[WSO2 Deployer Mojo] ---------------------------------------------------");
            try {
                getLog().info("[WSO2 Deployer Mojo] Load artifact '" + deploy.getArtifactId() + "' from repository ");
                final File artifact = loadArtifact(deploy);
                if (artifact != null) {
                    getLog().info(
                            "[WSO2 Deployer Mojo] Starting deployment: " + artifact.getName() + " to "
                                    + deploy.getServerId());

                    if ("war".equals(deploy.getArtifactType().toLowerCase())) {

                        final Wso2WarDeployer deployer =
                                new Wso2WarDeployer(getServerconfById(deploy.getServerId()), getLog());
                        getLog().info(
                                "[WSO2 Deployer Mojo] Deploy with version subcontext: " + deploy.isVersionSubContext());
                        ...
                        final Wso2AarDeployer deployer =
                                new Wso2AarDeployer(getServerconfById(deploy.getServerId()), getLog());
                        deployer.uploadAAR(artifact, deploy.getArtifactId() + ".aar", "");

                    } else if ("car".equals(deploy.getArtifactType().toLowerCase())) {

                        final Wso2CarDeployer deployer =
                                new Wso2CarDeployer(getServerconfById(deploy.getServerId()), getLog());
                        deployer.uploadCAR(artifact, deploy.getArtifactId() + ".car", "jar");
    ...
    private Serverconfig getServerconfById(final String serverId) {
    ...
    public File loadArtifact(final Deployment deploable) {
        File artifactFile = null;
        // set up atifact object
        final Artifact artifact =
                artifactFactory.createArtifactWithClassifier(deploable.getGroupId(), deploable.getArtifactId(),
                        deploable.getVersion(), deploable.getArtifactType(), deploable.getArtifactClassifier());
        try {
            // resolve artifact
